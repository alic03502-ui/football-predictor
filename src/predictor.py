...
Pipeline:
    ...
    -> goal markets, BTTS, team totals, clean sheets
"""
...
class PredictorBundle:
    ...
    def feature_row(
        ...
        row = {
            ...
            "pi_home_avg": pi_h_avg,
            "pi_away_avg": pi_a_avg,
            ...
            "h2h_home_pts": h2h["h2h_home_pts"],
            "h2h_home_gd": h2h["h2h_home_gd"],
            "h2h_n": h2h["h2h_n"],
    ...
    def predict(
        ...
        if stacked is not None:
            blended = {
                k: (1 - blend) * dc_probs[k] + blend * stacked[k]
                for k in "HDA"
            }
        ...
        # Re-scale the scoreline matrix so each H/D/A region
        # sums to the blended probability.
        sm_adj = sm.copy()
        ...
        # ------------------------------------------------------------
        # GOAL MARKETS
        # All goal markets are derived from the final adjusted
        # scoreline probability matrix.
        # ------------------------------------------------------------

        home_goal_values = np.arange(sm_adj.shape[0])
        away_goal_values = np.arange(sm_adj.shape[1])

        total_goals_grid = (
            home_goal_values[:, None] + away_goal_values[None, :]
        )

        # Total Goals O/U 0.5, 1.5, 2.5, 3.5, 4.5
        goals_over_under = {}

        for line in (0.5, 1.5, 2.5, 3.5, 4.5):
            over_probability = float(
                sm_adj[total_goals_grid > line].sum()
            )

            goals_over_under[str(line)] = {
                "over": over_probability,
                "under": float(1.0 - over_probability),
            }
        ...
        for line in (0.5, 1.5, 2.5):
            over_probability = float(
                sm_adj[home_goal_values > line, :].sum()
            )

            home_team_over_under[str(line)] = {
                "over": over_probability,
                "under": float(1.0 - over_probability),
            }
        ...
        for line in (0.5, 1.5, 2.5):
            over_probability = float(
                sm_adj[:, away_goal_values > line].sum()
            )

            away_team_over_under[str(line)] = {
                "over": over_probability,
                "under": float(1.0 - over_probability),
            }
        ...
        btts_yes = float(
            sm_adj[
                (home_goal_values[:, None] >= 1)
                & (away_goal_values[None, :] >= 1)
            ].sum()
        )
        ...
        # Clean sheets
        # Home clean sheet = away team scores 0
        # Away clean sheet = home team scores 0
        clean_sheet = {
            "home": float(sm_adj[:, 0].sum()),
            "away": float(sm_adj[0, :].sum()),
        }
        ...
        return {
            ...
            "pi_home_h": self.pi.get_h(home),
            "pi_away_a": self.pi.get_a(away),
            "lambda_home": lam,
            "lambda_away": mu,
            "dc_outcome": dc_probs,
            ...
            # Home team goal markets
            "home_team_over_under": home_team_over_under,

            # Away team goal markets
            "away_team_over_under": away_team_over_under,
...
def build_predict_state(
    ...
    history: dict[str, deque] = defaultdict(
        lambda: deque(maxlen=max_w)
    )

    xg_history: dict[str, deque] = defaultdict(
        lambda: deque(maxlen=max_w)
    )
    ...
    has_xg_col = (
        "home_xg" in matches.columns
        and "away_xg" in matches.columns
    )
    ...
    for row in matches.sort_values("date").itertuples(index=False):
        ...
        if has_xg_col:
            ...
            try:
                hxg = (
                    float(hxg)
                    if hxg is not None and not pd.isna(hxg)
                    else None
                )

                axg = (
                    float(axg)
                    if axg is not None and not pd.isna(axg)
                    else None
                )
        ...
        if hg > ag:
            last_streak[row.home] = (
                last_streak[row.home] + 1
                if last_streak[row.home] > 0
                else 1
            )

            last_streak[row.away] = (
                last_streak[row.away] - 1
                if last_streak[row.away] < 0
                else -1
            )
        ...
        rec["pts_first"] = float(
            np.mean([r[0] for r in rec["records"]])
        )

        rec["pts_second"] = float(
            np.mean([r[1] for r in rec["records"]])
        )

        rec["gd_first"] = float(
            np.mean([r[2] for r in rec["records"]])
        )
    ...
    for team, hist in history.items():
        ...
        if records:
            ...
            for w in WINDOWS:
                ...
                form[f"form{w}_pts"] = float(
                    np.mean([r[2] for r in recent])
                )

                form[f"form{w}_gd"] = float(
                    np.mean([r[0] - r[1] for r in recent])
                )

            # Rolling xG averages from last N games
            # where xG was recorded.
            xg_recs = [
                x
                for x in xg_history.get(team, [])
            ...
            for w in XG_WINDOWS:
                ...
                if len(recent_xg) >= max(2, w // 2):
                    form[f"xg_for{w}"] = float(
                        np.mean([x[0] for x in recent_xg])
                    )

                    form[f"xg_against{w}"] = float(
                        np.mean([x[1] for x in recent_xg])
                    )
    ...
    teams = sorted(
        set(matches["home"]).union(matches["away"])
    )
